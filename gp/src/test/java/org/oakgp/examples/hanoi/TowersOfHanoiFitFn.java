 ...
 */
class TowersOfHanoiFitFn implements FitFn {
    private static final TowersOfHanoi START_STATE = new TowersOfHanoi();

    private final boolean doLog;

    TowersOfHanoiFitFn(boolean doLog) {
        this.doLog = doLog;
    }
    ...
    public double doubleValueOf(Node n) {
        ...
        Set<TowersOfHanoi> previousStates = new UnifiedSet<>(4);
        previousStates.add(towersOfHanoi);

        Move previousMove = null;
        int previousFitness = Integer.MAX_VALUE-1; //penalty for length
        while (true) {
