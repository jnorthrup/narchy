...
public final class NodeReader implements Closeable {
     ...
     *
     * @param input       contains the s-expressions, representing programs as tree structures, that will be read to construct new {@code Node} instances
     * @param functions   a collection of {@code Function}s to use to represent functions read from {@code input}
    ...
    private static boolean notFunctionEnd(String token) {
    ...
    private static boolean notArrayEnd(String token) {
    ...
    private static ConstantNode createLongConstant(String token) {
    ...
    private static ConstantNode createBigDecimalConstant(String token) {
    ...
    private Map<Predicate<String>, java.util.function.Function<String, Node>> createReaders() {
        ...
        m.put("true"::equals, s -> TRUE_NODE);
        m.put("false"::equals, s -> FALSE_NODE);
        m.put("void"::equals, s -> VOID_CONSTANT);
        m.put(NodeReader::isLong, NodeReader::createLongConstant);
        m.put(NodeReader::isBigInteger, NodeReader::createBigIntegerConstant);
        m.put(NodeReader::isBigDecimal, NodeReader::createBigDecimalConstant);
        m.put(NodeReader::isDecimalNumber, NodeReader::createDoubleConstant);
        m.put(NodeReader::isNumber, NodeReader::createIntegerConstant);
    ...
    private Node createArrayConstantNode() throws IOException {
        ...
        while (notArrayEnd(nextToken = nextToken())) {
            ...
            if (t == null) {
                t = n.returnType();
            } else if (t != n.returnType()) {
                throw new IllegalStateException("Mixed type array elements: " + t + " and " + n.returnType());
            }
            arguments.add(n);
        }
