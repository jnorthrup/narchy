...
public interface SameSubtermsCompound extends Compound {
    ...
    default int intifyRecurse(int v, IntObjectToIntFunction<Term> reduce) {
        return reduce.intValueOf(subterms().intifyShallow(v, (v1, s) -> s.intifyRecurse(v1, reduce)), this);
    }
                                    ...
                                    (!decomposeConjDTernal) ?
                                            (when, what) -> {
                                                ...
                                                if (distributed.op() != CONJ)
                                                    throw new TermTransformException(SameSubtermsCompound.this, distributed, "invalid conjunction factorization"
                                                    );
...
//                break;
    ...
    default boolean hasXternal() {
        return dt() == XTERNAL || this.hasAny(Op.Temporal) && OR(Term::hasXternal);
    }
