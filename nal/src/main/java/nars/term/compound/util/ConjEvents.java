...
//    public final LongArrayList times = new LongArrayList(1);
    ...
    public final LongObjectHashMap<RoaringBitmap> event = new LongObjectHashMap<>(2);
...
//            }
            ...
            RoaringBitmap what = event.getIfAbsentPut(at, RoaringBitmap::new);
            if (what.contains(-id)) {
                //contradiction
                term = False;
                return false;
...
//    }
    ...
    private Term termConj(long w) {
        RoaringBitmap what = event.get(w);
        if (what!=null)
    ...
    private Term termConj(long w, RoaringBitmap what) {

        int n = what.getCardinality();
        if (n == 1) {
            //simple case
            return sub(what.first(), null);
        }

        final boolean[] negatives = {false};
        TreeSet<Term> t = new TreeSet();
        what.forEach((int termIndex)->{
            Term c = sub(termIndex, negatives);
            t.add(c);
        });
