...
public class ObjectEditorEventDecorator<KEY, ITEM> extends ObjectEditorDecoratorBase<KEY, ITEM> {
    ...
    final List<ModificationListener<KEY, ITEM>> listeners = new CopyOnWriteArrayList<>();
    ...
    public boolean compareAndUpdate( KEY key, String property, Object compare, Object value ) {
        fire( createModification( BEFORE_UPDATE, key, null, property, list( compare, value ) ) );
        boolean updated = super.compareAndUpdate( key, property, compare, value );
        if ( updated ) {
            fire( createModification( AFTER_UPDATE, key, null, property, list( compare, value ) ) );
        }
        return updated;
    }
