...
public class RepoBuilderDefault implements RepoBuilder {
     ...
     */
    final Map<String, Function> keyGetterMap = new HashMap();
     ...
     */
    private final Map<String, Comparator> collators = new HashMap<>();
     ...
     */
    private final Map<String, Function> keyTransformers = new HashMap<>();
     ...
     */
    private final Map<String, Integer> indexBucketSize = new HashMap<>();
    ...
    private <ITEM> void loadFields( Class<ITEM> clazz, Class<?>[] classes ) {
        ...
        for ( Class<?> cls : classes ) {
            ...
            for (Map.Entry<String, FieldAccess> stringFieldAccessEntry : fieldsComponentType.entrySet()) {
                if ( !fields.containsKey(stringFieldAccessEntry.getKey()) ) {
                    fields.put(stringFieldAccessEntry.getKey(), stringFieldAccessEntry.getValue());
                }
            }
        }
    }
    ...
    private void configIndexes( RepoComposer repo,
        ...
        for (Map.Entry<String, String[]> stringEntry : nestedIndexes.entrySet()) {
            NestedKeySearchIndex index = new NestedKeySearchIndex(stringEntry.getValue());
            configIndex(stringEntry.getKey(), index );
        }
    ...
    private void configSearchIndex( Map<String, FieldAccess> fields, String prop, SearchIndex searchIndex ) {
        ...
        if (comparator==null) {
            if (fieldAccess!=null && fieldAccess.type() == Typ.number) {
                comparator = (Comparator<Number>) (o1, o2) -> {
    ...
    private Function getKeyGetterOrCreate( Map<String, FieldAccess> fields,
        ...
        if (Typ.isMap(itemClass))  {
            ...
            if ( kg == null ) {
                ...
                keyGetterMap.put( prop, o -> BeanUtils.atIndex(o, prop));
            }
