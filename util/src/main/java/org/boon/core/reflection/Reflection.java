...
public class Reflection {
    ...
    static class Context {
        ...
        final Map<Class<?>, List<Field>> __fields = new ConcurrentHashMap<>( 200 );

        Unsafe control;
        final Map<String, String> _sortableFields = new ConcurrentHashMap<>();

        final Map<Class<?>, ClassMeta<?>> _classMetaMap = new ConcurrentHashMap<>( 200 );

        final Map<Class<?>, Map<String, FieldAccess>> _allAccessorReflectionFieldsCache = new ConcurrentHashMap<>( 200 );
        final Map<Class<?>, Map<String, FieldAccess>> _allAccessorPropertyFieldsCache = new ConcurrentHashMap<>( 200 );
        final Map<Class<?>, Map<String, FieldAccess>> _allAccessorUnsafeFieldsCache = new ConcurrentHashMap<>( 200 );

        final Map<Class<?>, Map<String, FieldAccess>> _combinedFieldsFieldsFirst = new ConcurrentHashMap<>( 200 );
        final Map<Class<?>, Map<String, FieldAccess>> _combinedFieldsFieldsFirstForSerializer = new ConcurrentHashMap<>( 200 );

        final Map<Class<?>, Map<String, FieldAccess>> _combinedFieldsPropertyFirst = new ConcurrentHashMap<>( 200 );
        final Map<Class<?>, Map<String, FieldAccess>> _combinedFieldsPropertyFirstForSerializer = new ConcurrentHashMap<>( 200 );

    }



    private static Map<String, FieldAccess> getCombinedFieldsPropertyFirst(Class<?> theClass) {
        return context()._combinedFieldsPropertyFirst.get(theClass);
    }


    private static Map<String, FieldAccess> getCombinedFieldsPropertyFirstForSerializer(Class<?> theClass) {
        return context()._combinedFieldsPropertyFirstForSerializer.get(theClass);
    }

    private static Map<String, FieldAccess> getCombinedFieldsFieldFirst(Class<?> theClass) {
        return context()._combinedFieldsFieldsFirst.get(theClass);
    }

    private static Map<String, FieldAccess> getCombinedFieldsFieldFirstForSerializer(Class<?> theClass) {
    ...
    private static void setAccessorFieldInCache(Class<?> theClass, boolean useUnsafe, Map<String, FieldAccess> map ) {
        if ( useUnsafe ) {
    ...
    private static void setPropertyAccessorFieldsInCache(Class<?> theClass, Map<String, FieldAccess> map ) {
        context()._allAccessorPropertyFieldsCache.put( theClass, map );
    }


    private static Map<String, FieldAccess> getPropertyAccessorFieldsFromCache( Class<?> theClass ) {
        return context()._allAccessorPropertyFieldsCache.get( theClass );
    }

    private static Map<String, FieldAccess> getAccessorFieldsFromCache(Class<?> theClass, boolean useUnsafe) {
    ...
    public static List<Field> getAllFields( Class<?> theClass ) {
    ...
    private static void extractProperty(Map<String, Pair<Method, Method>> methodMap,
        ...
        try {
            ...
            Pair<Method, Method> pair = methodMap.computeIfAbsent(propertyName, k -> new Pair<>());
