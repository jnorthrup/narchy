...
public class Tokenizer implements ITokenizer {
	...
	/*
	 * The state lookup map
	 */
	private final Map<Integer, TokenizerState> characterStates = new HashMap<>();
	...
	private final SlashState slashState = new SlashState();
	private final SymbolState symbolState = new SymbolState();
	private final WhitespaceState whitespaceState = new WhitespaceState();
	private final WordState wordState = new WordState();
	...
	private TokenizerState getCharacterState(int unicode) {
		return characterStates.get(unicode);
	}
	 ...
	 *            the string to read from
	 * 
	 * @param int the maximum length of a symbol, which establishes the size of
	 *        pushback buffer we need
	 */
	private void setString(String s, int symbolMax) {
	...
	/**
	 * From now on, all whitespace won't be swallowed but returned as tokens of
	 * type TT_WHITESPACE
	 */
	public void enableWhitespaceTokenizing() {
		whitespaceState().enableWhitespaceTokens();
	}
}
