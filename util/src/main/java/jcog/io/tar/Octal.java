...
class Octal {
    ...
    public static long parseOctal(byte[] header, int offset, int length) {
        ...
        for (int i = offset; i < end; ++i) {
            ...
            if ((int) header[i] == (int) (byte) ' ' || (int) header[i] == (int) '0') {
                if (stillPadding)
                    continue;

                if ((int) header[i] == (int) (byte) ' ')
                    break;
            }

            stillPadding = false;

            result = ( result << 3 ) + (long) ((int) header[i] - (int) '0');
        }
    ...
    public static int getOctalBytes(long value, byte[] buf, int offset, int length) {
        ...
        if (value == 0L) {
            ...
            for (long val = value; idx >= 0 && val > 0L; --idx) {
                buf[offset + idx] = (byte) ((int) (byte) '0' + (int) (byte) (val & 7L));
                val >>= 3L;
            }
        }
