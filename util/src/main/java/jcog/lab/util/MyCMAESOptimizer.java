...
public class MyCMAESOptimizer extends MultivariateOptimizer {
    ...
    private static int[] sortedIndices(final double[] doubles) {
        final MyCMAESOptimizer.DoubleIndex[] dis = new MyCMAESOptimizer.DoubleIndex[doubles.length];
        for (int i = 0; i < doubles.length; i++) {
            dis[i] = new MyCMAESOptimizer.DoubleIndex(doubles[i], i);
        }
        Arrays.sort(dis);
     ...
     * order.
     */
    private static class DoubleIndex implements Comparable<MyCMAESOptimizer.DoubleIndex> {
        /** Value to compare. */
    ...
    private class FitnessFunction {
        ...
        MyCMAESOptimizer.ValuePenaltyPair value(final double[] point) {
            ...
            if (isRepairMode) {
                ...
                value = MyCMAESOptimizer.this.computeObjectiveValue(repaired);
                penalty =  penalty(point, repaired);
            } else {
                value = MyCMAESOptimizer.this.computeObjectiveValue(point);
            }
            value = isMinimize ? value : -value;
            penalty = isMinimize ? penalty : -penalty;
            return new MyCMAESOptimizer.ValuePenaltyPair(value,penalty);
        }
