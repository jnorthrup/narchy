...
public class ObjectFilterTest {
    ...
    public void testCreateCriteriaFromJson() {
        ...
        equalsOrDie(Operator.EQUAL, foo.getOperator());
        equalsOrDie(foo.values[0], list.get(0));
        List<Operator> operators = Lists.list(Operator.LESS_THAN, Operator.EQUAL, Operator.NOT_EQUAL, Operator.GREATER_THAN, Operator.BETWEEN, Operator.IN, Operator.NOT_IN, Operator.CONTAINS, Operator.ENDS_WITH, Operator.STARTS_WITH, Operator.NOT_CONTAINS);
        List<TypeType> types = Lists.list(TypeType.INT, TypeType.DOUBLE, TypeType.OBJECT, TypeType.FLOAT, TypeType.SHORT, TypeType.BYTE, TypeType.LONG);
        for (Operator op : operators) {
            for (TypeType type : types) {
                List<Object> args = Lists.list("foo", op, type, Object.class, list);
                foo = (Criterion) Invoker.invokeFromList(ObjectFilter.class, "createCriteria", args);
        ...
        puts(object);
        foo = (Criterion) Invoker.invokeFromObject(ObjectFilter.class, "createCriteria", object);
        equalsOrDie(Operator.EQUAL, foo.getOperator());
        equalsOrDie(foo.values[0], list.get(0));
        equalsOrDie("foo", foo.getName());
        foo = (Criterion) ObjectFilter.createCriteriaFromClass("foo", org.boon.criteria.MyClass.class, Operator.EQUAL, list);
        equalsOrDie(Operator.EQUAL, foo.getOperator());
        equalsOrDie(foo.values[0], list.get(0));
        equalsOrDie("foo", foo.getName());
        foo = (Criterion) ObjectFilter.createCriteriaFromClass("bar.baz", org.boon.criteria.MyClass.class, Operator.EQUAL, list);
        equalsOrDie(Operator.EQUAL, foo.getOperator());
        equalsOrDie(foo.values[0], list.get(0));
        ...
        puts(object);
        foo = (Criterion) Invoker.invokeFromObject(ObjectFilter.class, "createCriteriaFromClass", object);
        equalsOrDie(Operator.EQUAL, foo.getOperator());
        equalsOrDie(foo.values[0], list.get(0));
        equalsOrDie("bar.baz", foo.getName());
        json = "[\"bar.baz\", \"org.boon.criteria.MyClass\", \"EQUAL\",  [1,2,3]]";
        List listJson = Lists.list("an example", "org.boon.criteria.MyClass", "AND", Lists.list(Lists.list("bar.baz", "EQUAL", Lists.list(1, 2, 3)), Lists.list("bar.baz", "EQUAL", Lists.list(1, 2, 3))));
        json = toJson(listJson);
        puts(json);
        object = fromJson(json);
        Group and = (Group) Invoker.invokeFromObject(ObjectFilter.class, "createCriteriaFromClass", object);
        puts(foo);
