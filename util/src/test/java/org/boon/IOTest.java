...
public class IOTest {
    ...
    public void testReadEachLineHttp() throws Exception {
        ...
        server.start();
        Thread.sleep(10);
        IO.eachLine("http://localhost:9668/test", new IO.EachLine() {
    ...
    public void testReadEachLineReader() throws Exception {
        File testDir = new File("src/test/resources/");
        File testFile = new File(testDir, "testfile.txt");
        IO.eachLine(new FileReader(testFile), new IO.EachLine() {
    ...
    public void testReadEachLineInputStream() throws Exception {
        File testDir = new File("src/test/resources");
        File testFile = new File(testDir, "testfile.txt");
        IO.eachLine(new FileInputStream(testFile), new IO.EachLine() {
    ...
    private void assertLines(List<String> lines) {
        assertEquals(4, len(lines));
        assertEquals("line 1", idx(lines, 0));
        assertEquals("grapes", idx(lines, 3));
    }
    ...
    public static class ProxyLoader {
        ...
        public ProxyLoader() throws URISyntaxException {
            this.dataFile = DATA_FILE;
            init();
        }

        public ProxyLoader(String dataFile) throws URISyntaxException {
            this.dataFile = DATA_FILE;
            init();
        }

        private void init() throws URISyntaxException {
            List<String> lines = IO.readLines(BoonTest.resource(dataFile));
        ...
        public static List<Proxy> loadProxies() throws URISyntaxException {
    ...
    public static final class Proxy2 {
        ...
        public static List<Proxy2> loadProxies() throws URISyntaxException {
            List<String> lines = IO.readLines(BoonTest.resource(DATA_FILE));
    ...
    public void proxyTest() throws URISyntaxException {
        ...
        assertEquals(5, len(proxyList));
        assertEquals("127.0.0.1", idx(proxyList, 0).getAddress());
        assertEquals(8080, idx(proxyList, 0).getPort());
        //192.55.55.57:9091
        assertEquals("192.55.55.57", idx(proxyList, -1).getAddress());
        assertEquals(9091, idx(proxyList, -1).getPort());
    }

    @Test
    public void proxyTest2() throws URISyntaxException {
    ...
    public void readClasspathResource() {
        ...
        //        grapes
        boolean ok = true;
        ok |= Str.in("apple", IO.read("classpath://testfile.txt")) || die("two slashes should work");
        //Proper URL
        ok |= Str.in("apple", IO.read("classpath:///testfile.txt")) || die("three slashes should work");
        //Not proper URL
        ok |= Str.in("apple", IO.read("classpath:testfile.txt")) || die("no slashes should work");
        //No URL
        ok |= Str.in("apple", IO.readFromClasspath(this.getClass(), "testfile.txt")) || die("you don't have to use classpath scheme");
        //Slash or no slash, it just works
        ok |= Str.in("apple", IO.readFromClasspath(this.getClass(), "/testfile.txt")) || die("on slash works");
        ...
        ok |= Lists.idx(paths, 0).endsWith("org" + File.separator + "node" + File.separator + "file1.txt") || die();
    }
}
