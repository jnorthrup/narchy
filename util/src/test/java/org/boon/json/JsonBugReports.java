...
public class JsonBugReports {
    ...
    public void testForIssue47() {
        ...
        puts(map);
        puts(toJson(map));
        boolean ok = toJson(map).equals("{\"docId\":111,\"empty\":\"\",\"serviceName\":\"cafe\"}") || die(toJson(map));
    }
    ...
    public void testForBug202_4() {
        String test = "{  \n" + "   \"timestamp\":1405673028,\n" + "   \"status\":200,\n" + "   \"request\":{  \n" + "      \"mbean\":\"com.openexchange.pooling:name=ConfigDB Read\",\n" + "      \"attribute\":\"MaxUseTime\",\n" + "      \"type\":\"read\"\n" + "   },\n" + "   \"value\":24\n" + "}";
        final JsonParserFactory jsonParserFactory = new JsonParserFactory();
        final Map<String, Object> stringObjectMap = jsonParserFactory.createJsonCharArrayParser().parseMap(test);
    }
}
