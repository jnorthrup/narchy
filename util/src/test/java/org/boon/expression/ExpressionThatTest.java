...
public class ExpressionThatTest {
    ...
    @BeforeEach
    public void setup() {
        BoonExpressionContext child = new BoonExpressionContext(Maps.map("name", "child-application", "session", Maps.map("name", "child-session", "sessionId", 2.44444d, "dukesBirthday", Dates.getUSDate(5, 29, 1970), "request", Maps.map("name", "child-request", "fruit", Lists.list("peaches", "strawberries"))), "SOME KEY", "SOME VALUE"));
        context = new BoonExpressionContext(child, Maps.map("name", "parent-application", "session", Maps.map("name", "parent-session", "request", Maps.map("name", "request-parent", "fruit", Lists.list("apple", "oranges")))));
    }
    ...
    @Test
    public void callFunctionOnParent() {
