    ...
    private static MeasuredRun test(final List<Employee> employees, final Map<String, List<MeasuredRun>> results) {
        return new MeasuredRun("test indexed", 1, 100, results) {
            ...
            protected void init() {
                ...
                repo = Repos.builder().primaryKey("id").searchIndex("firstName").lookupIndex("firstName").nestedIndex("department", "name").useUnsafe(true).removeDuplication(false).build(String.class, Employee.class);
                repo.addAll(copy(employees));
            }

            @Override
            protected void test() {
                repo.updateByFilter(Update.update(Update.incPercent("salary", 10)), eq("department.name", "engineering"));
            }
        };
    }
