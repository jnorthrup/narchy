...
public class ReflectionTest {
    ...
    public static Map<String, Object> bird = Maps.map("name", (Object) "bird", "friends", list("Tweety", "Chicken hawk", "Daffy", "Donald"));
    ...
    static Map<String, Object> department = map("name", (Object) "engineering", "employees", list(new Employee("Bob"), new Employee("Sue")));

    static Map<String, Object> department2 = map("name", (Object) "hr", "employees", set(new Employee("Bob"), new Employee("Sue"), new Employee("Sam")));

    static Map<String, Object> department3 = map("name", (Object) "manufacturing", "employees", set(map("firstName", "Rick", "id", "aaa"), map("firstName", "Tom", "id", "aaa"), map("firstName", "Chris", "id", "aaa"), map("firstName", "Diana", "id", "aaa")));
    ...
    public void testFromMapOfListOfMapsCauseIamHardcoreLikeThat() throws Exception {
        final Department manufacturing = Maps.fromMap(department3, Department.class);
        boolean ok = true;
        ...
        final Map<String, Object> oMapEmployee = (Map<String, Object>) BeanUtils.idx(objectMap.get("employees"), 0);
        ok &= oMapEmployee.get("firstName").equals("Rick") || die();
    }

    @Test
    public void testFromMap() throws Exception {
        final Department engineering = Maps.fromMap(department, Department.class);
        boolean ok = true;
        ok &= engineering.name.equals("engineering") || die();
        ok &= engineering.employees.size() == 2 || die();
        final Department hr = Maps.fromMap(department2, Department.class);
    ...
    public void test() throws Exception {
        ...
        ok &= Fields.getFirstComparableOrPrimitive(employee).equals("firstName") || die();
        ok &= Fields.getFirstComparableOrPrimitive(dog).equals("age") || die();
        ok &= Fields.getFirstComparableOrPrimitive(cat) == null || die();
        ok &= Fields.getFirstStringFieldNameEndsWith(employee, "Name").equals("firstName") || die();
        ok &= Fields.getFirstStringFieldNameEndsWith(cat, "Name") == null || die();
        ok &= Fields.hasStringField(employee, "firstName") || die();
        ok &= !Fields.hasStringField(employee, "bunnyShit") || die();
        ok &= Fields.getSortableField(employee).equals("firstName") || die();
        ok &= Fields.getSortableField(dog).equals("name") || die();
        try {
