...
class PriMergeTest {
    ...
    private static Prioritized testMerge(Prioritized x, Prioritized y, @NotNull PriMerge m, float ouPri) {
        return testMerge(new UnitPri(x), y, m, ouPri);
    }
    ...
    private static Prioritized testMerge(Prioritizable _x, Prioritized y, @NotNull PriMerge m, float ouPri, float expectedOverflow) {
        ...
        float overflow = m.merge(x, y.pri(), PriReturn.Overflow);
