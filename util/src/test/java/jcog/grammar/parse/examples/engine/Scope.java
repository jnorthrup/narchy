 ...
 * </code> and <code>Pop</code> have scope throughout
 * the rule.
 * <p>
 * The <code>bigCity</code> rule proves itself by finding a city and checking
 * its population. When the <code>city
 * </code> structure binds with a city fact in a
 * program, its variables take on the values of the fact. For example,
 * <code>city</code> might bind with the fact <code>
 * city(bigappolis, 8733352) </code>.
 * Then <code>Name</code> and <code>Pop</code> will bind to the values
 * "bigappolis" and 8733352.
 * <p>
 * When the comparison proves itself, it will compare <code>
 * Pop</code> to 1000000.
 * This is the same variable as <code>
 * Pop</code> in the <code>city</code> structure of
 * the rule. With this successful comparison, the rule completes a successful
 * proof, with <code>Name</code> bound to "bigappolis".
 * 
...
public class Scope implements PubliclyCloneable<Scope> {
	private Map<String, Variable> dictionary = new Hashtable<>();
	 ...
	 * 
	 * @param Term
	 *            [] the terms to seed this scope with
	 */
	...
	public Variable lookup(String name) {
		Variable v = dictionary.get(name);
		if (v == null) {
			v = new Variable(name);
			dictionary.put(v.name, v);
		}
