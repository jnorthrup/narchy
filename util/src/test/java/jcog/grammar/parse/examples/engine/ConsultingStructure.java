...
/**
 * A ConsultingStructure is structure that can prove itself against an axiom
 * source supplied with the constructor.
 * <p>
 * When a rule creates a dynamic version of itself, a normal structure will
 * create a ConsultingStructure counterpart that uses the rule's scope, and that
 * consults a given axiom source (such as a Program).
 * <p>
 * The primary behavior that a ConsultingStructure adds is the ability to prove
 * itself.
 * 
...
public class ConsultingStructure extends Structure {
	...
	/*
	 * Returns the axioms that a consulting structure can consult. Note that
	...
	public boolean canFindNextProof() {
		...
		/*
		 * A consulting structure proves itself by unifying with rule in a
		 * program. When that rule has more than one structure, the proving
		 * structure takes the tail of the rule as its "resolvent". The
		 * resolvent is the remainder of the rule, which, if proven true,
		 * confirms the truth of this structure. The resolvent may have multiple
		 * different proofs, and each of these counts as a new proof of this
		 * structure.
		 */
		if (resolvent != null) {
		...
		while (true) {
			/*
			 * No hogging the CPU! Who knows? We might be in a tight loop that
			 * the user wants to halt. For example, with the program
			 * "loop :- loop;" and the query "loop", we need to distract the
			 * proof dog enough to let a halt button click come through.
			 */
			Thread.yield();
			/*
			...
			/*
			 * Show that the unifying axiom's remainder is either empty or
			 * provable.
			 */
	 ...
	 * 
	 * <p>
	 * For example, the following structures can unify: <blockquote>
	 * 
	 * <pre>
