...
public class SlingPanel extends JPanel {
    ...
    private void calculateCorners(double aspectRatio) {
		 ...
		 */
		lowerLeft = new Point((w - 1) / 2 - (thickness - 1) / 2, (h - 1) / 2 + (tallness - 1) / 2);

		upperRight = new Point((w - 1) / 2 + (thickness - 1) / 2, (h - 1) / 2 - (tallness - 1) / 2);
	}
	...
	public void paintComponent(Graphics g) {
		...
		while (e.hasMoreElements()) {
			...
			Scale s = new Scale(ex.min, f, ex.max, lowerLeft, upperRight);
