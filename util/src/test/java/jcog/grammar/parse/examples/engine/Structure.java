 ...
 * <p>
 * An example of a structure is: <blockquote>
 * 
 * <pre>
 * starred(jamesCagney, &quot;Yankee Doodle Dandy&quot;, Year)
 * </pre>
 * 
 * </blockquote> This structure has the String <code>"starred"</code> as its
 * functor. This structure's first term is another structure that has
 * "jamesCagney" as its functor and no terms of its own. Similarly, the second
 * term is a structure with the functor "Yankee Doodle Dandy" and no terms of
 * its own. The third term is a variable, Year.
 * <p>
 * This particular example has two elements that favor a parser: the quotes
 * around the film title and the capitalization of the variable. When using
 * Structure and Variable directly, you do not need the kinds of clues a parser
 * needs. So, Yankee Doodle Dandy is just another string, whose internal blanks
 * are not at all confusing. Further, a variable can have any string as its
 * name, not necessarily capitalized.
 * <p>
 * You can create the starred example with <blockquote>
 * 
 * <pre>
 * Structure s = new Structure(&quot;starred&quot;, new Term[] { new Structure(&quot;jamesCagney&quot;), new Structure(&quot;Yankee Doodle Dandy&quot;),
 * 		new Variable(&quot;Year&quot;) });
 * </pre>
 * 
 * </blockquote> To be able to prove itself against a program, a structure must
 * appear in a Rule. Rules associate like-named variables in a "scope", which is
 * essentially a dictionary. A rule makes an executable copy of itself by
 * creating a new variable dictionary, and by making "consulting" copies of its
 * structures.
 * 
...
public class Structure implements Term {
	 ...
	 * 
	 * @param Term
	 *            [] the terms of the structure, which may be either variables
	 *            or other structures
	 */
	 ...
	 * <p>
	 * Objects of this class, the superclass of all structures, should not
	 * appear in dynamic rules. When a nondynamic rule creates its dynamic
	 * counterpart, it populates it with provable versions of its structures. A
	 * general <code>Structure</code> object will construct a <code>
 * ConsultingStructure</code>
	 * when it participates in building a dynamic rule.
	 * <p>
	 * This particular method is almost never called. Subclasses implement more
	...
	/**
	 * Return this structure, if it is nonatomic, or just the functor, if this
	 * is an atom.
	 */
	public Object eval() {
		if (terms().length > 0) {
	...
	/**
	 * Returns <code>true</code> if this structure's functor and number of terms
	 * match the supplied structure.
	 * 
	 * @param Structure
	 *            the structure to compare this one against
	 * 
	 ...
	 * 
	 * A list is a structure whose functor is "." and that has two terms. The
	 * first term of a list can be any term; the second term of a list is
	 * another list, an empty list, or a variable. <p> This method accepts an
	 * array of terms and a tail, which must be a list structure or a variable.
	 * This method composes and returns a two-element array. <p> The first
	 * element of the returned array will always be the first element of the
	 * supplied terms array. <p> The second element of the returned array will
	...
	/**
	 * Return true, if this structure is a list, which means it has an functor
	 * of ".", and has two terms, the second of which must be a list.
	 * 
	 * @return true if this structure is a list
	 */
	public boolean isList() {
		return terms.length == 2 && functor.equals(".") && terms[1].isList();
	}
	 ...
	 * <p>
	 * This constructor creates a list of two terms, regardless of the number of
	 * terms supplied here. The new list's first term is the first term of the
	 ...
	 * 
	 * @param Term
	 *            [] the leading terms of the list. In practice, this array
	 *            usually contains a single term.
	 * 
	 * @param Term
	 *            a list, or a variable that represents the tail of the list
	 */
	...
	/**
	 * Returns a representation of this list as the inner part of some other
	 * list. This method is used by <code>toString()
 * </code>.
	 */
	...
	/*
	 * Return a textual represenation of this list's terms, with a normal
	...
	/**
	 * Unifies the terms in this structure with the terms in the given
	 * structure, and returns the variable bindings that result.
	 * <p>
	 * If two structures have equal functors and the same number of terms, they
	 * can unify if all of their terms unify. For example, the following
	 * structures can unify: <blockquote>
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * 
	 * </blockquote> The unification of these structures is: <blockquote>
	 * 
	 * <pre>
	 *     Detail = mall(fayette),
	 *     City = lexington,
	 ...
	 * <p>
	 * This method dispatches the unify request to the variable. Note that the
	 * variable may be instantiated to a structure that contains variables. An
	 * uninstantiated variable will bind to this structure, but an instantiated
	 * variable will forward the unification request to its instantiation.
	 * 
	 * @param Term
	 *            a term to unify with
	 * 
	 ...
	 * This method adds this structure's variables directly to the returned
	 * unification. In addition, this method adds in all the variables from the
	 * structures among this structure's terms.
	 * <p>
	 * For example, the variables of: <blockquote>
	 * 
	 * <pre>
	 * address(street(StreetName), city(CityName), State)
	 * </pre>
	 * 
	 * </blockquote> are StreetName, CityName, and State.
	 * 
	 * @return unification all the variables of the terms of this structure
	 */
