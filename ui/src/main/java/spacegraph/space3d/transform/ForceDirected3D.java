...
public class ForceDirected3D implements spacegraph.space3d.phys.constraint.BroadConstraint {
    ...
    public void solve(Broadphase b, List<Collidable> objects, float timeStep) {
        ...
        for (int iter = 0; iter < iterations; iter++) {
            ...
            objects.stream().map(new Function<Collidable, Spatial>() {
                @Override
                public Spatial apply(Collidable c) {
                    return ((Spatial) c.data());
                }
            }).filter(Objects::nonNull).forEach(new Consumer<Spatial>() {
                @Override
