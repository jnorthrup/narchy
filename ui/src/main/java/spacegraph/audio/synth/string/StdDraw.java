...
final class StdDraw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
     ...
     */
    private static final Color BOOK_RED = new Color(150, 35, 31);
     ...
     */
    private static void setCanvasSize(int w, int h) {
     ...
     * The pen radius is not affected by coordinate scaling.
     */
    private static void setPenRadius() {
     ...
     */
    private static void setPenRadius(double r) {
     ...
     */
    private static void setPenColor(Color color) {
        penColor = color;
        offscreen.setColor(penColor);
    }
     ...
     * @throws IllegalArgumentException if the radius of the circle is negative
     */
    private static void arc(double x, double y, double r, double angle1, double angle2) {
     ...
     */
    private static void square(double x, double y, double r) {
     ...
     * @throws IllegalArgumentException if r is negative
     */
    private static void filledSquare(double x, double y, double r) {
        if (r < 0) throw new IllegalArgumentException("square side length must be nonnegative");
        double xs = scaleX(x);
        double ys = scaleY(y);
        double ws = factorX(2 * r);
     ...
     * @param y an array of all the y-coordinates of the polygon
     */
    private static void filledPolygon(double[] x, double[] y) {
        int N = x.length;
        GeneralPath path = new GeneralPath();
        path.moveTo((float) scaleX(x[0]), (float) scaleY(y[0]));
        for (int i = 0; i < N; i++)
     ...
     * @param s the text
     */
    private static void text(double x, double y, String s) {
