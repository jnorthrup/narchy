...
public abstract class UGen extends Auvent {
     ...
     */
    private FasterList<BufferPointer>[] inputsAtChannel;

    /**
     * A collection of UGens that should be triggered by this one.
     */
    private final CopyOnWriteArrayList<UGen> dependents;
    ...
    public UGen(AudioContext context, int ins, int outs) {
        dependents = new CopyOnWriteArrayList<>();
        noInputs = true;
        lastTimeStep = -1;
     ...
     */
    @SuppressWarnings("unchecked")
    public List<UGen> getDependents() {
