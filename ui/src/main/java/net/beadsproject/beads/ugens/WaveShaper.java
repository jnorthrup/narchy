...
public class WaveShaper extends UGen implements DataBeadReceiver {
    ...
    private float preGain = 2.0F;
    private float postGain = 1.0F;
    private float limit = 1.0F;
    private float wetMix = 1.0F;
     ...
     *
     * @param context     The audio context.
     * @param shapeTensor The Buffer from which to get the wave shape.
     */
    public WaveShaper(AudioContext context, ArrayTensor shapeTensor) {
        this(context);
        setShape(shapeTensor.data);
    }
     ...
     * @param shapeTensor The Buffer from which to get the wave shape.
     */
    public WaveShaper(AudioContext context, int channels, ArrayTensor shapeTensor) {
        this(context, channels);
        setShape(shapeTensor.data);
    }
    ...
    private static float[] generateCosineShape(int length) {
        ...
        for (int i = 1; i < length - 1; i++) {
            ret[i] = (float) 0 - (float) Math.cos((double) (float) i * Math.PI / (double) l);
        }
        ret[0] = -1.0F;
        ret[length - 1] = 1.0F;
        if (length % 2 == 1) {
    ...
    public static float[] generateExponentialShape(int length, float exponent) {
        ...
        int l = length - 1;
        for (int i = 1; i < length - 1; i++) {
            float x = (((float) i) / (float) l) * 2f - 1.0F;
            if (x < (float) 0) {
                ret[i] = (float) -Math.pow((double) -x, (double) exponent);
            } else if (x == (float) 0) {
                ret[i] = (float) 0;
            } else {
                ret[i] = (float) Math.pow((double) x, (double) exponent);
    ...
    public void gen() {
        ...
        if (channels == 1) {
            ...
            for (int currsample = 0; currsample < bufferSize; currsample++) {
                ...
                if (y1 <= (float) 0) {
                    y2 = shape[0] * postGain;
                } else if (y1 >= (float) shapeLen) {
                    y2 = shape[shapeLen] * postGain;
                } else {
                    int ind = (int) y1;
                    float frac = y1 - (float) ind;
                    y2 = (shape[ind] * (1.0F - frac) + shape[ind + 1] * frac)
                            * postGain;
                }
                ...
                bo[currsample] = y * (1.0F - wetMix) + y2 * wetMix;
            }
            ...
            for (int currsample = 0; currsample < bufferSize; currsample++) {
                ...
                for (int currchannel = 0; currchannel < channels; currchannel++) {
                    ...
                    bufOut[currchannel][currsample] = y * (1.0F - wetMix) + y2
                            * wetMix;
                }
            }
        }
    ...
    public UGen getPreGainUGen() {
        return isPreGainStatic ? null : preGainUGen;
    }
    ...
    public UGen getPostGainUGen() {
        return isPostGainStatic ? null : postGainUGen;
    }
    ...
    public UGen getLimitUGen() {
        return isLimitStatic ? null : limitUGen;
    }
    ...
    public UGen getWetMixUGen() {
        return isWetMixStatic ? null : wetMixUGen;
    }
