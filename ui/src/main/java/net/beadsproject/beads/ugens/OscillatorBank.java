...
public class OscillatorBank extends UGen {
     ...
     * @param tensor         the buffer used as a lookup table by the oscillators.
     * @param numOscillators the number of oscillators.
     */
    public OscillatorBank(AudioContext context, Tensor tensor, int numOscillators) {
        super(context, 1);
        this.tensor = tensor;
        setNumOscillators(numOscillators);
        gain = 1f / numOscillators;
    }
    ...
    public void setNumOscillators(int numOscillators) {
        ...
        System.arraycopy(old, 0, gains, 0, min);
        for (int i = min; i < gains.length; i++) {
            gains[i] = 1f;
        }
        old = point;
        point = new float[numOscillators];
        System.arraycopy(old, 0, point, 0, min);
    ...
    public void setFrequencies(float[] frequencies) {
        for (int i = 0; i < numOscillators; i++) {
            frequency[i] = i < frequencies.length ? Math.abs(frequencies[i]) : 0f;
            increment[i] = frequency[i] / context.getSampleRate();
        }
    }
    ...
    public void setGains(float[] gains) {
        for (int i = 0; i < numOscillators; i++) {
            this.gains[i] = i < gains.length ? gains[i] : 0f;
        }
    }
    ...
    public void calculateBuffer() {
        ...
        for (int i = 0; i < bufferSize; i++) {
            for (int j = 0; j < numOscillators; j++) {
                ...
                bufOut[0][i] += gains[j] * tensor.getFractInterp(point[j]);
            }
            bufOut[0][i] *= gain;
        }
    }
