...
public class SV_CCMDS {
	...
	*/
	public static void SV_WriteLevelFile() {
		...
		var name = FS.Gamedir() + "/save/current/" + SV_INIT.sv.name + ".sv2";

		try {
			var f = new QuakeFile(name, "rw");
	...
	*/
	public static void SV_WriteServerFile(boolean autosave) {
		...
		var filename = FS.Gamedir() + "/save/current/server.ssv";
		try {
	...
	public static void SV_GameMap_f() {
		...
		if (map.charAt(0) == '*') {
			...
			if (SV_INIT.sv.state == Defines.ss_game) {
				...
				client_t cl;
				var savedInuse = new boolean[(int) SV_MAIN.maxclients.value];
				for (var i = 0; i < SV_MAIN.maxclients.value; i++) {
	...
	*/
	public static void SV_Status_f() {
		...
		for (var i = 0; i < SV_MAIN.maxclients.value; i++) {
			...
			var s = NET.AdrToString(cl.netchan.remote_address);
			Com.Printf(s);
			l = 22 - s.length();
			for (j = 0; j < l; j++)
				Com.Printf(" ");
