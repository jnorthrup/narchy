...
public abstract class Model extends Surf {
	...
	*/
	...
	static final FloatBuffer globalModelTextureCoordBuf = Lib.newFloatBuffer(MODEL_BUFFER_SIZE * 2);
	static final ShortBuffer globalModelVertexIndexBuf = Lib.newShortBuffer(MODEL_BUFFER_SIZE);
	...
	static void precompileGLCmds(qfiles.dmdl_t model) {
		...
		var tmp = new Vector<Integer>();

		var count = 0;
		var order = model.glCmds;
		var orderIndex = 0;
		while (true)
		{
		...
		count = 0;
		var pos = 0;
		var indexElements = model.indexElements;
