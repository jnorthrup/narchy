...
public abstract class Warp extends Model {
    ...
    void EmitWaterPolys(msurface_t fa) {
        ...
        if ((fa.texinfo.flags & Defines.SURF_FLOWING) != 0)
            scroll = -64.0F
                    * ((r_newrefdef.time * 0.5f) - (float) (int) (r_newrefdef.time * 0.5f));
        else
            scroll = (float) 0;
        float t = (float) 0;
        float s = (float) 0;
    ...
    final float[][] skyclip = { {1.0F, 1.0F, (float) 0}, {1.0F, -1.0F, (float) 0}, {(float) 0, -1.0F, 1.0F}, {(float) 0, 1.0F, 1.0F},
            {1.0F, (float) 0, 1.0F}, {-1.0F, (float) 0, 1.0F} };
    ...
    final int[][] vec_to_st = { { -2, 3, 1 }, { 2, 3, -1 },
    ...
    void DrawSkyPolygon(int nump, float[][] vecs) {
        ...
        float[] av = {(float) 0, (float) 0, (float) 0};
        av[0] = Math.abs(v[0]);
        av[1] = Math.abs(v[1]);
        av[2] = Math.abs(v[2]);
        int axis;
    ...
    final float[] dists = new float[MAX_CLIP_VERTS];

    final int[] sides = new int[MAX_CLIP_VERTS];
    ...
    final float[][] verts = new float[MAX_CLIP_VERTS][3];
    ...
    void MakeSkyVec(float s, float t, int axis) {
        float[] b = {(float) 0, (float) 0, (float) 0};
