...
public class Art {
    ...
    private static Image getImage(GraphicsConfiguration gc, String imageName) throws IOException {
        var source = ImageIO.read(Art.class.getResourceAsStream(imageName));
    ...
    private static Image[][] cutImage(GraphicsConfiguration gc, String imageName, int xSize, int ySize) throws IOException {
        var source = getImage(gc, imageName);
        var images = new Image[source.getWidth(null) / xSize][source.getHeight(null) / ySize];
        for (var x = 0; x < source.getWidth(null) / xSize; x++) {
            for (var y = 0; y < source.getHeight(null) / ySize; y++) {
