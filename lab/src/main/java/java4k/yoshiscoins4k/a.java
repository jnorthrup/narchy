...
public class a extends Applet implements Runnable {
    ...
    public void run() {
		...
		for (y = 0; y < 232; y++) {
			for (x = 0; x < 7; x++) {
                float percent = (float) y / 231f;
				g.setColor(new Color(Color.HSBtoRGB(0.58f, 0.88f - 0.29f * percent, (y <= 75 || (x & 1) == 1 ? 1.0F : 0.9f) * (0.7f + 0.3f * percent))));
        ...
        for (y = 0; y < 3; y++) {
			for (x = 0; x < 4; x++) {
				W[y][x] = W[5 - y][x] = W[5 - y][6 - x] = W[y][6 - x] = 0x0F & ((int) S.charAt(y) >> (x << 2));
			}
		}
        ...
        for (z = 0; z < 2; z++) {
			for (i = 0; i < 8; i++) {
				...
				for (j = 0; j < 3; j++) {
					for (y = 0; y < 24; y++) {
						for (x = 0; x < 8; x++) {
                            float dx = (float) (x + (j << 3)) - 11.5f;
                            float dy = (float) y - 11.5f;
							k = (int) (dx * dx + dy * dy);
							pixels[x] = (k >= 144) ? 0 : (0xFF000000 | palette[(z << 2) + (0x03 & ((int) S.charAt(3 + y + j * 24 + i * 72) >> (x << 1)))]);
						}
		...
		for (i = 0; i < 8; i++) {
			...
			for (j = 0; j < 2; j++) {
				for (y = 0; y < z; y++) {
					for (x = 0; x < 8; x++) {
						k = 0x03 & ((int) S.charAt(579 + y + j * z + (i << 5)) >> (x << 1));
        ...
        while (true) {
			...
			do {
                ...
                switch (state) {
                    ...
                    case STATE_EVALUATE_MOVE:
                        ...
                        if (board[7][1] != NODE_INTERMEDIATE) {
                            if (board[7][1] != NODE_TIE) {
                                ...
                                y = board[7][3];
                                for (i = 0; i < 4; i++) {
                                    bouncingCoins[i][COIN_X] = (float) (1 + x * 26);
                                    bouncingCoins[i][COIN_FLOOR_Y] = bouncingCoins[i][COIN_Y] = (float) (77 + y * 26);
                                    bouncingCoins[i][COIN_VY] = COIN_JUMP_VY;
                                    bouncingCoins[i][COIN_DELAY] = (float) (i << 2);
                                    bouncingCoins[i][COIN_SPRITE] = (float) (spriteBoard[y][x] - 1);
                                    spriteBoard[y][x] = 0;
                                    if (board[7][4] < x) {
                                        x--;
                                    } else if (board[7][4] > x) {
                    ...
                    case STATE_BOUNCING:
                        for (i = 0; i < 4; i++) {
                            if (bouncingCoins[i][COIN_DELAY] > (float) 0) {
                                bouncingCoins[i][COIN_DELAY]--;
                            } else {
                                bouncingCoins[i][COIN_VY] += COIN_JUMP_GRAVITY;
                                bouncingCoins[i][COIN_Y] += bouncingCoins[i][COIN_VY];
                                if (bouncingCoins[i][COIN_VY] > (float) 0 && bouncingCoins[i][COIN_Y] >= bouncingCoins[i][COIN_FLOOR_Y]) {
			...
			if (fading) {
				...
				for (y = 116 - fadeRadius; y < 116 + fadeRadius; y++) {
					x = 116 - y;
					z = (int) Math.sqrt((double) (fadeRadius * fadeRadius - x * x));
