...
public class a extends Applet implements Runnable {
	...
	private static void n(int[][] board, int[][] sorts, int depth, int stone, int alpha, int beta, int maxDepth, int[][] permutations, int[] result) {
        ...
        for (var i = 0; i < 7; i++) {
			...
			var largestColumn = -1;
			for (var j = 0; j < 7; j++) {
				var column = permutations[depth][j];
    ...
    public void run() {
		...
		var backgroundImage = new BufferedImage(182, 232, 1);
		var g = (Graphics2D) backgroundImage.getGraphics();
		for (y = 0; y < 232; y++) {
			for (x = 0; x < 7; x++) {
				var percent = y / 231f;
		...
		var sprites = new BufferedImage[24];
		var pixels = new int[8];
        int k;
        int j;
        int i;
        int z;
        int[] palette = {0xB88818, 0xE0C028, 0xF8F040, 0xFFFFFF, 0xB71919, 0xDD2727, 0xF74040, 0xFF7575,};
        for (z = 0; z < 2; z++) {
			for (i = 0; i < 8; i++) {
		...
		var blinkIndex = 0;
		var blinkCountdown = 0;
		var staring = false;
		var stareCountdown = 0;
		var mouseReleased = true;
		var xPlaysFirst = true;
        ...
        int[][] sorts = null;
        final var SPRITE_WIN = 23;
        final var SPRITE_STARE = 22;
        final var SPRITE_LOSE = 21;
        final var SPRITE_BLINK_1 = 19;
        final var SPRITE_HEAD = 18;
        final var SPRITE_BODY = 17;
        final var SPRITE_EGG = 16;
        final var COIN_FLOOR_Y = 5;
        final var COIN_SPRITE = 4;
        final var COIN_DELAY = 3;
        final var COIN_VY = 2;
        final var COIN_Y = 1;
        final var COIN_X = 0;
        final var STATE_O_PLAYS_FIRST = 7;
        final var STATE_TIE = 6;
        final var STATE_BOUNCING = 5;
        final var STATE_EVALUATE_MOVE = 4;
        final var STATE_O_CHOOSING = 3;
        final var STATE_WAIT_FOR_PAINT = 2;
        final var STATE_COIN_DROPPING = 1;
        final var STATE_X_CHOOSING = 0;
        final var MOUSE_PRESSED = 1;
        final var MOUSE_X = 0;
        final var COIN_JUMP_GRAVITY = 0.05f;
        final var COIN_JUMP_VY = -0.8f;
        final var GRAVITY = 0.5f;
        while (true) {
