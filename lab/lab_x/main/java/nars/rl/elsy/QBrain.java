...
public class QBrain implements Serializable {
     ...
     */
    public QBrain(Perception perception, Action[] actionsArray, int[] hiddenNeuronsNo, double alpha, double lambda, double gamma, boolean useBoltzmann, double temperature, double randActions, double maxWeight) {
     ...
     * @param hiddenNeuronsNo - numbers of neurons in hidden layers
     */
    public QBrain(Perception perception, Action[] actionsArray, int[] hiddenNeuronsNo) {
        this(
                perception,
                actionsArray,
                hiddenNeuronsNo,
     ...
     * @param actionsArray - array of actions that can be taken
     */
    public QBrain(Perception perception, Action[] actionsArray) {
        this(
                perception,
                actionsArray,
                new int[]{} 
    ...
    private int selectAction() {
        ...
        for (int i = 0; i < Q.length; i++) {
            if (useBoltzmann) {
                boltzValues[i] = exp(Q[i] / temperature);
            }
            if (Qmax < Q[i]) {
                a = i;
                Qmax = Q[a];
    ...
    public void inheritFrom(QBrain brain, QBrain brain2) {
        for (int l = 0; l < w.length; l++) {
            for (int i = 0; i < w[l].length; i++) {
    ...
    public void printStats() {
        ...
        avgWght /= wghtNo;
        out.println("Max=" + maxWght + " Avg=" + avgWght + " No=" + wghtNo);
    }
